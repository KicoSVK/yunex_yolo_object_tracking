...
def start_websocket_communication(uri):
    asyncio.set_event_loop(asyncio.new_event_loop())
    asyncio.get_event_loop().run_until_complete(send_data(uri))

if __name__ == "__main__":
    server_uri = "ws://localhost:8765"

    # Create a thread for WebSocket communication
    websocket_thread = threading.Thread(target=start_websocket_communication, args=(server_uri,))
